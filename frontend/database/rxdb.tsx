...
) => [RxReplicationState<any, any>, number, BehaviorSubject<boolean>, BehaviorSubject<boolean>] = <
...
>(
  ...
  const tokensReplicationState = replicateRxCollection({
    ...
    pull: {
      handler: async (lastCheckpoint: any, batchSize: any): Promise<any> => {
        pulling$.next(true);
        try {
          const documentsFromRemote = await pullFunc(
            lastCheckpoint ? lastCheckpoint.updatedAt : 0,
            lastCheckpoint?.id,
            batchSize,
          );
  ...
  return [tokensReplicationState, setInterval(() => tokensReplicationState.reSync(), 15000), pushing$, pulling$];
};
...
export class RxdbDatabase extends IWalletDatabase {
  ...
  async init(): Promise<void> {
    ...
    [this.tokensReplicationState, this.tokensPullInterval, this.pushingTokens$, this.pullingTokens$] = setupReplication<
      TokenRxdbDocument,
      string
    >(
      tokens,
