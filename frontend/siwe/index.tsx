...
import type { LoginOkResponse, SignedDelegation as _SignedDelegation } from "./service.interface";
...
export function SiweIdentityProvider({
  ...
  const { signMessage, data, status: signMessageStatus, reset, error: signMessageError } = useSignMessage();
  ...
  useEffect(() => {
    (async () => {
      ...
      if (state.loginStatus === "logging-in") {
        ...
        try {
          loginOkResponse = await callLogin(state.anonymousActor, data, address, sessionPublicKey);
        } catch (e) {
          if (e instanceof Error) {
            console.error(e);
          }
